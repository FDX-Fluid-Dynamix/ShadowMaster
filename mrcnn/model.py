...
class BatchNorm(KL.BatchNormalization):
    """Extends the Keras BatchNormalization class to allow a central placeto make changes if needed.
        Batch normalization has a negative effect on training if batches are small so this layer is often frozen (via setting in Config class) and functionsas linear layer.
    """
    def call(self, inputs, training=None):
        """Note about training values: None: Train BN layers. This is the normal mode
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    box_indices = tf.range(0, tf.shape(roi_masks)[0])
...
class DetectionTargetLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # Slice the batch and run a graph for each slice  Rename target_bbox to target_deltas for clarity
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    detections = tf.concat([
        ...
        tf.to_float(tf.gather(class_ids, keep))[..., tf.newaxis],tf.gather(class_scores, keep)[..., tf.newaxis]], axis=1)
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # Negative ROIs are those with max IoU 0.1-0.5 (hard example mining)  To hard example mine or not to hard example mine, that's the question
...
#%%
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # 2. GT Boxes in pixels (zero padded)  [batch, MAX_GT_INSTANCES, (y1, x1, y2, x2)] in image coordinates
        ...
        # Returns a list of the last layers of each stage, 5 in total. Don't create the thead (stage 5), so we pick the 4th item in the list.
        if callable(config.BACKBONE):
        ...
        # Concatenate layer outputs Convert from list of lists of level outputs to list of lists of outputs across levels.
        ...
        if mode == "training":
            ...
            # Network Heads
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox =fpn_classifier_graph(rois, mrcnn_feature_maps, input_image_meta,config.POOL_SIZE, config.NUM_CLASSES,
...
##            
            ...
            model = KM.Model([input_image, input_image_meta, input_anchors], [detections, mrcnn_class, 
                             mrcnn_bbox,  mrcnn_mask, rpn_rois, rpn_class, rpn_bbox], name='mask_rcnn')
##
...
#%%
    def compile(self, learning_rate, momentum):
        ...
        # Add LossesFirst, clear previously set losses to avoid duplication
        ...
        # Add L2 Regularization Skip gamma and beta weights of batch normalization layers.
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers, augmentation=None, custom_callbacks=None, no_augmentation_sources=None):
        ...
        """      layers: Allows selecting wich layers to train. It can be:
            ...
            - One of these predefined values:
              ...
              all: All the layers
              3+: Train Resnet stage 3 and up, 4+: Train Resnet stage 4 and up, 5+: Train Resnet stage 5 and up
        ...
        # Work-around for Windows: Keras fails on Windows when using multiprocessing workers. See discussion here:
        ...
        self.keras_model.fit_generator( train_generator, initial_epoch=self.epoch, epochs=epochs,
            steps_per_epoch=self.config.STEPS_PER_EPOCH, callbacks=callbacks, validation_data=val_generator,
            validation_steps=self.config.VALIDATION_STEPS,  max_queue_size=100,
...
#%%
    def mold_inputs(self, images):
        ...
        windows = []
        for image in images:
            # Resize image  TODO: move resizing to mold_image()
            molded_image, window, scale, padding, crop = utils.resize_image( image, min_dim=self.config.IMAGE_MIN_DIM,
                min_scale=self.config.IMAGE_MIN_SCALE, max_dim=self.config.IMAGE_MAX_DIM, mode=self.config.IMAGE_RESIZE_MODE)
...
#%%    
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape, image_shape, window):
        ...
        """
        # How many detections do we have? Detections array is padded with zeros. Find the first class_id == 0.
...
# =============================================================================
    ...
    def detect(self, images, verbose=0):
        ...
        # Validate image sizes, All images in a batch MUST be of the same size
