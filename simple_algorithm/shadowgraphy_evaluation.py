"""
Algorithm to detect the drop size from shadowgraphy recordings.
...
if os.path.isdir(evaluation_path) == True:
    ...
    raise ValueError(
        "The folder for the final result: %s does not exist!!! " % evaluation_path)
...
Detecion_class = functions.DROP_DETECTION(
    scale, min_droplet_size, min_ratio, min_mw, min_dif)
...
if int(settings_correct) == 1:
    raise ValueError(
        " Cancellation by user because incorrect settings were selected.")
...
for i_folder, subfolder in enumerate(subfolders):
    ...
    if sub_bg == True:
        Preprocessing.bg_erzeugen(
            fileNames,  n_pic_bg, evaluation_folder, pic_name)

    else:
        Preprocessing.bg_pic = 255 * \
            np.ones((Detecion_class.shape1, Detecion_class.shape2))
    ...
    for i_pic, fileName in enumerate(fileNames[0:file_end]):
        ...
        img_n = cv2.normalize(orimg, None, alpha=0, beta=255,
        ...
        drops_pro_pic_f,  image_bin_filtered = Detecion_class.filter_circles(
            img_n, image_bin_fill,  drops_pro_pic, plot_circ)
        image_bin_filtered = image_bin_filtered.astype(np.uint8)
        radius = drops_pro_pic_f.diameters/2

        ######################################  SECOND FILTER ####################################################################################

        drops_pro_pic_c, contours, area, ratio = Detecion_class.contour_filter(
            img_n, image_bin_filtered, scale, plot_cont)

        image_with_drops = img_n.copy()
        image_with_drops = cv2.drawContours(
            image_with_drops, contours, -1, (255, 0, 0), thickness=-1)
        ...
        if plot_all_steps:
            plot_func.plot_all_steps(i_pic, orimg, image_blur, image_bin_fill, image_bin_filtered,
                                     drops_pro_pic_f.centroids, radius, image_with_drops, save_img, evaluation_folder, pic_name)
        if plotten_erg:
            plot_func.plot_final_result(
                i_pic, orimg, image_with_drops, save_img, evaluation_folder, pic_name)

        plt.close('all')
        t2 = cv2.getTickCount()
        print("Time for image %i : %.3f s" %
              (i_pic+1, (t2-t1)/cv2.getTickFrequency()))
    ...
    if postproc:
        ...
        plot_func.post_processing(
            evaluation_folder, drop_max, name, DROPS.r_g_c, i_pic+1, title_his)

        plt.close('all')
        name = pic_name+'labeled_filter'
        title_his = r'Drop size in  $\mu$m'
        plot_func.post_processing(
            evaluation_folder, drop_max, name, DROPS.r_g, i_pic+1, title_his)
